...
describe('Component Hub services', () => {
  ...
  it('loads the component catalog from the API', () => {
    ...
    expect(catalog.getById('date-range-picker')?.name).toBe(
      'Date Range Picker',
    );
  ...
  it('loads and creates workspaces through the component API', () => {
    ...
    http
      .expectOne('/api/components/notification-banner/workspaces')
      .flush({ workspaces: [] });
    ...
    const request = http.expectOne(
      '/api/components/notification-banner/workspaces',
    );
